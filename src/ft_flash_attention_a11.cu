...
template <int HeadDim>
__global__ __launch_bounds__(kThreads) void ft_flash_attention_a11_kernel(
...
#pragma unroll
    ...
    recovered_u0 += __shfl_xor_sync(0xffffffffU, recovered_u0, 1, 2);
    recovered_u1 += __shfl_xor_sync(0xffffffffU, recovered_u1, 1, 2);
    recovered_u2 += __shfl_xor_sync(0xffffffffU, recovered_u2, 1, 2);
    if (storage.recovery_action[row] != 0) {
...
int main(int argc, char** argv) {
  ...
  if (options.head_dim == 64) {
    CUDA_CHECK(cudaFuncSetAttribute(ft_flash_attention_a11_kernel<64>,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    sizeof(AttentionSharedStorage<64>)));
  } else {
    CUDA_CHECK(cudaFuncSetAttribute(ft_flash_attention_a11_kernel<128>,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    sizeof(AttentionSharedStorage<128>)));
  }
  auto launch = [&](FaultStage fault_stage, cudaStream_t stream) {
    if (options.head_dim == 64) {
      ft_flash_attention_a11_kernel<64>
      ...
      ft_flash_attention_a11_kernel<128>
  ...
  std::printf("version: FT-FlashAttention A11 recovery-only verifier recompute\n");
