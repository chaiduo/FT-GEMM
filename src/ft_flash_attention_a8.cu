...
struct alignas(128) AttentionSharedStorage {
  ...
  alignas(128) Element p[cute::cosize_v<typename Traits::SmemLayoutP>];
  alignas(128) Element v[cute::cosize_v<typename Traits::SmemLayoutV>];
  alignas(128) float score[kScoreStride * kKeyTile];
  alignas(128) float pv[kScoreStride * HeadDim];
...
template <int HeadDim>
__global__ __launch_bounds__(kThreads) void ft_flash_attention_a8_kernel(
  ...
  auto sP = cute::make_tensor(cute::make_smem_ptr(storage.p), typename Traits::SmemLayoutP{});
  ...
  using PvLayout = decltype(cute::make_layout(
      ...
      cute::make_stride(cute::_1{}, cute::Int<kScoreStride>{})));
  auto sScore = cute::make_tensor(cute::make_smem_ptr(storage.score), ScoreLayout{});
  auto sPv = cute::make_tensor(cute::make_smem_ptr(storage.pv), PvLayout{});
...
#pragma unroll
    ...
    for (int index = tid; index < kQueryTile * kKeyTile; index += kThreads) {
      ...
      float score = active ? storage.score[local_key * kScoreStride + local_row] * score_scale
                           : -FLT_MAX;
      if (active && fault_stage == kFaultScore && batch_id == fault_batch &&
          head == fault_head && global_query == fault_query && global_key == fault_index) {
        score += fault_value;
      }
      storage.score[local_key * kScoreStride + local_row] = score;
    }
    __syncthreads();
    ...
    for (int local_key = pair_lane; local_key < row_valid_keys; local_key += 2) {
      float score = storage.score[local_key * kScoreStride + row];
    ...
    if (storage.recovery_action[row] == 1 && pair_lane == 0) {
      ...
      storage.score[local_key * kScoreStride + row] -= storage.recovery_delta[row];
    } else if (storage.recovery_action[row] == 2) {
      for (int local_key = pair_lane; local_key < row_valid_keys; local_key += 2) {
        float replay_score = 0.0f;
        for (int dim = 0; dim < HeadDim; ++dim) {
          replay_score +=
              static_cast<float>(sQ(row, dim)) * static_cast<float>(sK(local_key, dim));
        }
        storage.score[local_key * kScoreStride + row] = replay_score * score_scale;
      }
    }
    __syncthreads();
    ...
    for (int local_key = pair_lane; local_key < kKeyTile; local_key += 2) {
      ...
      if (local_key < row_valid_keys) {
        int key_index = key_begin + local_key;
        primary_probability =
            __expf(storage.score[local_key * kScoreStride + row] - new_m) *
            kProbabilityScale;
        verifier_probability =
            __expf(storage.score[local_key * kScoreStride + row] - new_verifier_m) *
            kProbabilityScale;
      ...
      sP(row, local_key) = primary_quantized;
    ...
    if (storage.recovery_action[row] != 0) {
      ...
      for (int local_key = begin; local_key < end; local_key += step) {
        float verifier_probability =
            __expf(storage.score[local_key * kScoreStride + row] - new_verifier_m) *
            kProbabilityScale;
        sP(row, local_key) = Element(verifier_probability);
      }
    }
    __syncthreads();
    ...
    for (int local_key = pair_lane; local_key < row_valid_keys; local_key += 2) {
      int key_index = key_begin + local_key;
      float probability = static_cast<float>(sP(row, local_key));
      float weight = position_weight(key_index, sequence);
      float verifier_probability =
          static_cast<float>(Element(__expf(
              storage.score[local_key * kScoreStride + row] - new_verifier_m) *
                                     kProbabilityScale));
    ...
    for (int index = tid; index < kQueryTile * HeadDim; index += kThreads) {
      ...
      storage.output[dim * kScoreStride + local_row] =
          storage.stage_alpha[local_row] *
              storage.output[dim * kScoreStride + local_row] +
          storage.pv[dim * kScoreStride + local_row];
    }
    __syncthreads();
  }
...
int main(int argc, char** argv) {
  ...
  if (options.head_dim == 64) {
    CUDA_CHECK(cudaFuncSetAttribute(ft_flash_attention_a8_kernel<64>,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    sizeof(AttentionSharedStorage<64>)));
  } else {
    CUDA_CHECK(cudaFuncSetAttribute(ft_flash_attention_a8_kernel<128>,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    sizeof(AttentionSharedStorage<128>)));
  }
  auto launch = [&](FaultStage fault_stage, cudaStream_t stream) {
    if (options.head_dim == 64) {
      ft_flash_attention_a8_kernel<64>
      ...
      ft_flash_attention_a8_kernel<128>
  ...
  std::printf("version: FT-FlashAttention A8 multi-query dual-WGMMA\n");
