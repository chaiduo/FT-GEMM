...
static Options parse_options(int argc, char** argv) {
  ...
  if (options.m <= 0 || options.n <= 0 || options.k <= 0 ||
      ...
      options.verify_samples < 0 || options.abs_tolerance < 0.0f ||
      options.rel_tolerance <= 0.0f) {
...
__device__ __forceinline__ bool checksum_bad(
    float actual, float expected, float scale, float abs_tolerance,
    float rel_tolerance) {
  return fabsf(actual - expected) >
         abs_tolerance + rel_tolerance * fmaxf(1.0f, scale);
}
...
__global__ void tile_abft_kernel(
    const Element* a, const Element* b, float* c, int m, int n, int k,
    int tile_rows, int tile_cols, float abs_tolerance, float rel_tolerance,
    bool inject_fault, int* bad_tiles, int* corrected_tiles) {
  ...
  if (threadIdx.x == 0 && threadIdx.y < valid_rows) {
    ...
    for (int kk = 0; kk < k; ++kk) {
      ...
      for (int local_col = 0; local_col < valid_cols; ++local_col) {
        float value = static_cast<float>(
            b[kk * n + tile_col * kTile + local_col]);
        b_sum += value;
    ...
    if (checksum_bad(actual, expected, scale, abs_tolerance,
                     rel_tolerance)) {
  ...
  if (threadIdx.y == 0 && threadIdx.x < valid_cols) {
    ...
    for (int kk = 0; kk < k; ++kk) {
      ...
      for (int local_row = 0; local_row < valid_rows; ++local_row) {
        float value = static_cast<float>(
            a[(tile_row * kTile + local_row) * k + kk]);
        a_sum += value;
        a_abs_sum += fabsf(value);
      }
      float bvalue = static_cast<float>(
          b[kk * n + tile_col * kTile + threadIdx.x]);
  ...
  if (threadIdx.x == 0 && threadIdx.y == 0) {
    ...
    if (bad_row_count == 1 && bad_col_count == 1) {
      ...
      c[fault_row * n + fault_col] +=
          0.5f * (row_deltas[bad_row] + col_deltas[bad_col]);
...
int main(int argc, char** argv) {
  ...
  auto enqueue_gemm = [&]() {
    native_tiled_fp8_gemm_kernel<<<grid, block, 0, stream>>>(
        d_a, d_b, d_c, options.m, options.n, options.k);
  };
  auto enqueue_abft = [&]() {
    tile_abft_kernel<<<tile_count, block, 0, stream>>>(
        d_a, d_b, d_c, options.m, options.n, options.k, tile_rows, tile_cols,
        options.abs_tolerance, options.rel_tolerance, options.inject_fault,
        d_bad_tiles, d_corrected_tiles);
  };
