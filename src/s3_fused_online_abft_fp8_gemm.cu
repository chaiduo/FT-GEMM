...
__global__ void __cluster_dims__(1, 1, 1)
    wgmma_tma_fused_abft_kernel(CUTLASS_GRID_CONSTANT TmaA const tma_a, CUTLASS_GRID_CONSTANT TmaB const tma_b, float* c, int* bad_tiles,
  ...
  bool is_tma_leader = (threadIdx.x < 32) && static_cast<bool>(cute::elect_one_sync());
  ...
  if (is_tma_leader) {
    for (int stage = 0; stage < kStages; ++stage) {
      cutlass::arch::ClusterTransactionBarrier::init(&storage.full_barrier[stage], 1);
    }
  ...
  if (is_tma_leader) {
    cutlass::arch::ClusterTransactionBarrier::arrive_and_expect_tx(&storage.full_barrier[0], kTmaTransactionBytes);
    cute::copy(tma_a.with(storage.full_barrier[0]), tAgA(cute::_, cute::_, cute::_, 0), tAsA(cute::_, cute::_, cute::_, 0));
    cute::copy(tma_b.with(storage.full_barrier[0]), tBgB(cute::_, cute::_, cute::_, 0), tBsB(cute::_, cute::_, cute::_, 0));
  }
  ...
  for (int k_tile = 0; k_tile < k_tile_count; ++k_tile) {
    ...
    cutlass::arch::ClusterTransactionBarrier::wait(&storage.full_barrier[read_stage], read_phase);
    ...
    if (next_k_tile < k_tile_count) {
      if (is_tma_leader) {
        ...
        cutlass::arch::ClusterTransactionBarrier::arrive_and_expect_tx(&storage.full_barrier[write_stage], kTmaTransactionBytes);
        cute::copy(tma_a.with(storage.full_barrier[write_stage]), tAgA(cute::_, cute::_, cute::_, next_k_tile),
                   tAsA(cute::_, cute::_, cute::_, write_stage));
        cute::copy(tma_b.with(storage.full_barrier[write_stage]), tBgB(cute::_, cute::_, cute::_, next_k_tile),
  ...
  if (threadIdx.x == 0) {
    ...
    if (storage.bad_row_count == 1 && storage.bad_col_count == 1) {
      storage.c[storage.bad_col * kM + storage.bad_row] +=
          0.5f * (storage.row_deltas[storage.bad_row] + storage.col_deltas[storage.bad_col]);
...
int main(int argc, char** argv) {
  ...
  auto enqueue_gemm = [&]() {
    ...
    config.numAttrs = 1;
    CUDA_CHECK(cudaLaunchKernelEx(&config, kernel, tma_a, tma_b, d_c, d_bad_tiles, d_corrected_tiles, options.m, options.n, options.k,
                                  tile_rows, tile_cols, options.abft_abs_tolerance, options.abft_rel_tolerance, options.fault_value,
