...
static Options parse_options(int argc, char** argv) {
  ...
  options.autotune_repeat = get_int(argc, argv, "--autotune-repeat", options.autotune_repeat);
  options.verify_samples = get_int(argc, argv, "--verify-samples", options.verify_samples);
  options.workspace_mb = get_int(argc, argv, "--workspace-mb", options.workspace_mb);
  options.peak_tflops = get_double(argc, argv, "--peak-tflops", options.peak_tflops);
  ...
  if (options.device < 0 || options.m <= 0 || options.n <= 0 || options.k <= 0 || options.warmup < 0 || options.repeat <= 0 ||
      options.autotune_repeat <= 0 || options.verify_samples < 0 || options.workspace_mb < 0 || options.peak_tflops <= 0.0) {
...
__global__ void generate_fp8_kernel(__nv_fp8_e4m3* output, size_t count, uint32_t seed) {
...
static VerificationResult verify_samples(const std::vector<__nv_fp8_e4m3>& a, const std::vector<__nv_fp8_e4m3>& b,
  ...
  for (int row = 0; row < options.m && checked_rows < options.verify_samples; row += row_step, ++checked_rows) {
    ...
    for (int col = 0; col < options.n && checked_cols < options.verify_samples; col += col_step, ++checked_cols) {
      ...
      for (int kk = 0; kk < options.k; ++kk) {
        reference += static_cast<double>(static_cast<float>(a[static_cast<size_t>(row) * options.k + kk])) *
                     static_cast<double>(static_cast<float>(b[kk + static_cast<size_t>(col) * options.k]));
      }
      float actual = c[row + static_cast<size_t>(col) * options.m];
      float absolute_error = static_cast<float>(std::fabs(actual - reference));
      float tolerance = 0.5f + 2.0e-2f * std::max(1.0f, static_cast<float>(std::fabs(reference)));
...
int main(int argc, char** argv) {
  ...
  size_t workspace_bytes = static_cast<size_t>(options.workspace_mb) * 1024 * 1024;
  ...
  CUDA_CHECK(cudaMemcpy(d_scales, host_scales, sizeof(host_scales), cudaMemcpyHostToDevice));
  ...
  CUBLAS_CHECK(cublasLtMatmulDescSetAttribute(operation, CUBLASLT_MATMUL_DESC_TRANSA, &transa, sizeof(transa)));
  CUBLAS_CHECK(cublasLtMatmulDescSetAttribute(operation, CUBLASLT_MATMUL_DESC_TRANSB, &transb, sizeof(transb)));
  const void* a_scale = d_scales;
  const void* b_scale = d_scales + 1;
  CUBLAS_CHECK(cublasLtMatmulDescSetAttribute(operation, CUBLASLT_MATMUL_DESC_A_SCALE_POINTER, &a_scale, sizeof(a_scale)));
  CUBLAS_CHECK(cublasLtMatmulDescSetAttribute(operation, CUBLASLT_MATMUL_DESC_B_SCALE_POINTER, &b_scale, sizeof(b_scale)));
  if (options.fast_accum) {
    int8_t fast_accum = 1;
    CUBLAS_CHECK(cublasLtMatmulDescSetAttribute(operation, CUBLASLT_MATMUL_DESC_FAST_ACCUM, &fast_accum, sizeof(fast_accum)));
  }

  CUBLAS_CHECK(cublasLtMatrixLayoutCreate(&a_layout, CUDA_R_8F_E4M3, options.k, options.m, options.k));
  CUBLAS_CHECK(cublasLtMatrixLayoutCreate(&b_layout, CUDA_R_8F_E4M3, options.k, options.n, options.k));
  CUBLAS_CHECK(cublasLtMatrixLayoutCreate(&c_layout, CUDA_R_32F, options.m, options.n, options.m));
  CUBLAS_CHECK(cublasLtMatmulPreferenceCreate(&preference));
  CUBLAS_CHECK(cublasLtMatmulPreferenceSetAttribute(preference, CUBLASLT_MATMUL_PREF_MAX_WORKSPACE_BYTES, &workspace_bytes,
                                                    sizeof(workspace_bytes)));

  constexpr int max_algorithms = 32;
  std::vector<cublasLtMatmulHeuristicResult_t> heuristics(max_algorithms);
  int returned_results = 0;
  CUBLAS_CHECK(cublasLtMatmulAlgoGetHeuristic(handle, operation, a_layout, b_layout, c_layout, c_layout, preference, max_algorithms,
  ...
  auto run_algorithm = [&](int algorithm_index) {
    return cublasLtMatmul(handle, operation, &alpha, d_a, a_layout, d_b, b_layout, &beta, d_c, c_layout, d_c, c_layout,
                          &heuristics[algorithm_index].algo, workspace, workspace_bytes, stream);
  };
  ...
  for (int algorithm = 0; algorithm < returned_results; ++algorithm) {
    ...
    for (int iteration = 0; iteration < options.autotune_repeat; ++iteration) {
  ...
  if (options.verify_samples > 0) {
    ...
    h_b.resize(b_count);
    h_c.resize(c_count);
    CUDA_CHECK(cudaMemcpy(h_a.data(), d_a, a_count * sizeof(*d_a), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(h_b.data(), d_b, b_count * sizeof(*d_b), cudaMemcpyDeviceToHost));
    CUDA_CHECK(cudaMemcpy(h_c.data(), d_c, c_count * sizeof(*d_c), cudaMemcpyDeviceToHost));
    verification = verify_samples(h_a, h_b, h_c, options);
  }
  bool passed = options.verify_samples == 0 || verification.max_tolerance_ratio <= 1.0f;

  int algorithm_id = -1;
  size_t algorithm_id_size = sizeof(algorithm_id);
  CUBLAS_CHECK(cublasLtMatmulAlgoConfigGetAttribute(&heuristics[best_algorithm].algo, CUBLASLT_ALGO_CONFIG_ID, &algorithm_id,
                                                    sizeof(algorithm_id), &algorithm_id_size));
  ...
  std::printf("types: A=FP8 E4M3 B=FP8 E4M3 accumulator=FP32 output=FP32\n");
