...
struct alignas(128) AttentionSharedStorage {
  ...
  union alignas(128) {
    ...
    Element p[cute::cosize_v<typename Traits::SmemLayoutP>];
  } key_probability;
...
template <int HeadDim>
__global__ __launch_bounds__(kThreads) void ft_flash_attention_a9_kernel(
  ...
  auto sP = cute::make_tensor(cute::make_smem_ptr(storage.key_probability.p), typename Traits::SmemLayoutP{});
  ...
  using PvLayout = decltype(cute::make_layout(
      ...
      cute::make_stride(cute::_1{}, cute::Int<kScoreStride>{})));
  auto sScore = cute::make_tensor(cute::make_smem_ptr(storage.score_pv.score), ScoreLayout{});
  auto sPv = cute::make_tensor(cute::make_smem_ptr(storage.score_pv.pv), PvLayout{});
...
int main(int argc, char** argv) {
  ...
  if (options.head_dim == 64) {
    CUDA_CHECK(cudaFuncSetAttribute(ft_flash_attention_a9_kernel<64>,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    sizeof(AttentionSharedStorage<64>)));
  } else {
    CUDA_CHECK(cudaFuncSetAttribute(ft_flash_attention_a9_kernel<128>,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    sizeof(AttentionSharedStorage<128>)));
  }
  auto launch = [&](FaultStage fault_stage, cudaStream_t stream) {
    if (options.head_dim == 64) {
      ft_flash_attention_a9_kernel<64>
      ...
      ft_flash_attention_a9_kernel<128>
  ...
  std::printf("version: FT-FlashAttention A9 aliased-smem dual-WGMMA\n");
