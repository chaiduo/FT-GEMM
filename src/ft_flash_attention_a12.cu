...
constexpr int kKeysPerWarpgroup = kKeyTile / kConsumerWarpgroups;
constexpr int kLanesPerRow = kThreads / kQueryTile;
constexpr int kScoreStride = kQueryTile + 1;
constexpr float kProbabilityScale = 256.0f;

using QkMmaShape = cute::Shape<cute::_64, cute::_64, cute::_32>;
...
struct AttentionTraits {
  ...
  using SmemLayoutV = decltype(cute::tile_to_shape(
      cute::SM90::GMMA::Layout_K_INTER_Atom<Element>{},
      cute::Shape<cute::Int<HeadDim>, cute::_64>{}));
};
...
struct alignas(128) AttentionSharedStorage {
  ...
  union alignas(128) {
    Element k[kConsumerWarpgroups][cute::cosize_v<typename Traits::SmemLayoutK>];
    Element p[kConsumerWarpgroups][cute::cosize_v<typename Traits::SmemLayoutP>];
  } key_probability;
  alignas(128) Element v[kConsumerWarpgroups][cute::cosize_v<typename Traits::SmemLayoutV>];
  union alignas(128) {
    float score[kScoreStride * kKeyTile];
    float pv[kConsumerWarpgroups][kScoreStride * HeadDim];
  } score_pv;
...
__device__ __forceinline__ float row_group_sum(float value) {
  value += __shfl_xor_sync(0xffffffffU, value, 2, kLanesPerRow);
  value += __shfl_xor_sync(0xffffffffU, value, 1, kLanesPerRow);
  return value;
}

__device__ __forceinline__ float row_group_max(float value) {
  value = fmaxf(value, __shfl_xor_sync(0xffffffffU, value, 2, kLanesPerRow));
  value = fmaxf(value, __shfl_xor_sync(0xffffffffU, value, 1, kLanesPerRow));
  return value;
}
...
template <int HeadDim>
__global__ __launch_bounds__(kThreads) void ft_flash_attention_a12_kernel(
  ...
  auto sK = cute::make_tensor(cute::make_smem_ptr(storage.key_probability.k[consumer_group]),
                              typename Traits::SmemLayoutK{});
  auto sP0 = cute::make_tensor(cute::make_smem_ptr(storage.key_probability.p[0]),
                               typename Traits::SmemLayoutP{});
  auto sP1 = cute::make_tensor(cute::make_smem_ptr(storage.key_probability.p[1]),
                               typename Traits::SmemLayoutP{});
  auto sP = consumer_group == 0 ? sP0 : sP1;
  auto sV = cute::make_tensor(cute::make_smem_ptr(storage.v[consumer_group]),
  ...
  auto sScore = cute::make_tensor(
      cute::make_smem_ptr(storage.score_pv.score + consumer_group * kKeysPerWarpgroup * kScoreStride), ScoreLayout{});
  auto sPv = cute::make_tensor(
      cute::make_smem_ptr(storage.score_pv.pv[consumer_group]), PvLayout{});

  int row = tid / kLanesPerRow;
  int row_lane = tid % kLanesPerRow;
  ...
  for (int key_begin = 0; key_begin < key_limit; key_begin += kKeyTile) {
    ...
    for (int index = consumer_thread; index < kKeysPerWarpgroup * HeadDim; index += 128) {
      int local_key = index / HeadDim;
      int dim = index % HeadDim;
      int key_index = key_begin + consumer_group * kKeysPerWarpgroup + local_key;
    ...
    if (row_valid_keys > 0) {
      ...
      for (int dim = row_lane; dim < HeadDim; dim += kLanesPerRow) {
        float qval = static_cast<float>(sQ(row, dim));
        ...
        if (row_valid_keys == full_tile_keys) {
          ...
          for (int local_key = 0; local_key < row_valid_keys; ++local_key) {
            float kval = static_cast<float>(k[qkv_offset(batch_id, head, key_begin + local_key, dim, heads, sequence, HeadDim)]);
    ...
    expected_sum = row_group_sum(expected_sum);
    expected_weighted = row_group_sum(expected_weighted);
    expected_scale = row_group_sum(expected_scale);
    expected_weighted_scale = row_group_sum(expected_weighted_scale);
    if (row_lane == 0) {
    ...
    auto score_accumulator =
        cute::partition_fragment_C(qk_mma, cute::make_shape(cute::_64{}, cute::_64{}));
...
#pragma unroll
    for (int dim_tile = 0; dim_tile < HeadDim / 32; ++dim_tile) {
      ...
      auto k_tile = cute::local_tile(
          sK, cute::Shape<cute::_64, cute::_32>{}, cute::make_coord(0, dim_tile));
    ...
    for (int local_key = row_lane; local_key < row_valid_keys; local_key += kLanesPerRow) {
    ...
    actual_score_sum = row_group_sum(actual_score_sum);
    actual_score_weighted = row_group_sum(actual_score_weighted);
    if (row_lane == 0) {
    ...
    if (storage.recovery_action[row] == 1 && row_lane == 0) {
      ...
      for (int local_key = row_lane; local_key < row_valid_keys; local_key += kLanesPerRow) {
        ...
        for (int dim = 0; dim < HeadDim; ++dim) {
          replay_score +=
              static_cast<float>(sQ(row, dim)) * static_cast<float>(k[qkv_offset(batch_id, head, key_begin + local_key, dim, heads, sequence, HeadDim)]);
        }
    ...
    verifier_max = row_group_max(verifier_max);
    ...
    for (int local_key = row_lane; local_key < kKeyTile; local_key += kLanesPerRow) {
      ...
      if (local_key < kKeysPerWarpgroup) {
        sP0(row, local_key) = primary_quantized;
      } else {
        sP1(row, local_key - kKeysPerWarpgroup) = primary_quantized;
      }
    ...
    verifier_u0 = row_group_sum(verifier_u0);
    verifier_tile_u1 = row_group_sum(verifier_tile_u1);
    verifier_tile_u2 = row_group_sum(verifier_tile_u2);
    negative_count = row_group_sum(negative_count);
    ...
    if (storage.recovery_action[row] != 0) {
      int begin =
          storage.recovery_action[row] == 1 ? storage.recovery_index[row] : row_lane;
      int end =
          storage.recovery_action[row] == 1 ? begin + 1 : row_valid_keys;
      int step = storage.recovery_action[row] == 1 ? 1 : kLanesPerRow;
      for (int local_key = begin; local_key < end; local_key += step) {
        float verifier_probability =
            __expf(storage.score_pv.score[local_key * kScoreStride + row] - new_verifier_m) *
            kProbabilityScale;
        if (local_key < kKeysPerWarpgroup) {
          sP0(row, local_key) = Element(verifier_probability);
        } else {
          sP1(row, local_key - kKeysPerWarpgroup) = Element(verifier_probability);
        }
      }
    }
    __syncthreads();

    float recovered_u0 = 0.0f;
    float recovered_u1 = 0.0f;
    float recovered_u2 = 0.0f;
    if (storage.recovery_action[row] != 0) {
      for (int local_key = row_lane; local_key < row_valid_keys; local_key += kLanesPerRow) {
        float probability = local_key < kKeysPerWarpgroup
                                ? static_cast<float>(sP0(row, local_key))
                                : static_cast<float>(sP1(row, local_key - kKeysPerWarpgroup));
        float weight = position_weight(key_begin + local_key, sequence);
        recovered_u0 += probability;
        recovered_u1 += weight * probability;
        recovered_u2 += weight * weight * probability;
      }
    }
    recovered_u0 = row_group_sum(recovered_u0);
    recovered_u1 = row_group_sum(recovered_u1);
    recovered_u2 = row_group_sum(recovered_u2);
    if (storage.recovery_action[row] != 0) {
      actual_u0 = recovered_u0;
      actual_u1 = recovered_u1;
      actual_u2 = recovered_u2;
    }
    ...
    tile_expected_output_sum = row_group_sum(tile_expected_output_sum);
    tile_expected_output_weighted = row_group_sum(tile_expected_output_weighted);
    ...
    cute::warpgroup_arrive();
#pragma unroll
    for (int key_tile = 0; key_tile < kKeysPerWarpgroup / 32; ++key_tile) {
    ...
    for (int index = tid; index < kQueryTile * HeadDim; index += kThreads) {
      ...
      storage.output[dim * kScoreStride + local_row] =
          storage.stage_alpha[local_row] *
              storage.output[dim * kScoreStride + local_row] +
          storage.score_pv.pv[0][dim * kScoreStride + local_row] +
          storage.score_pv.pv[1][dim * kScoreStride + local_row];
    }
    __syncthreads();
  }
  ...
  actual_output_sum = row_group_sum(actual_output_sum);
  actual_output_weighted = row_group_sum(actual_output_weighted);
  if (row_lane == 0) {
    storage.recovery_action[row] = 0;
    storage.recovery_index[row] = -1;
    storage.recovery_delta[row] = 0.0f;
...
int main(int argc, char** argv) {
  ...
  if (options.head_dim == 64) {
    CUDA_CHECK(cudaFuncSetAttribute(ft_flash_attention_a12_kernel<64>,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    sizeof(AttentionSharedStorage<64>)));
  } else {
    CUDA_CHECK(cudaFuncSetAttribute(ft_flash_attention_a12_kernel<128>,
                                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                                    sizeof(AttentionSharedStorage<128>)));
  }
  auto launch = [&](FaultStage fault_stage, cudaStream_t stream) {
    if (options.head_dim == 64) {
      ft_flash_attention_a12_kernel<64>
      ...
      ft_flash_attention_a12_kernel<128>
  ...
  std::printf("version: FT-FlashAttention A12 split-key dual-warpgroup WGMMA\n");
