...
__global__ void __cluster_dims__(1, 1, 1) wgmma_tma_cached_abft_kernel(
    CUTLASS_GRID_CONSTANT TmaA const tma_a,
    CUTLASS_GRID_CONSTANT TmaB const tma_b,
    const float* a_sums, const float* a_magnitudes,
    const float* b_sums, const float* b_magnitudes, float* c,
  ...
  int checksum_index =
      is_row_checksum_thread ? threadIdx.x : threadIdx.x - kM;
...
#pragma unroll
      for (int kk = 0; kk < kK; ++kk) {
        float avalue =
            static_cast<float>(sA_stage(checksum_index, kk));
        expected_checksum += avalue * storage.stage_b_sums[kk];
        checksum_scale +=
            fabsf(avalue) * storage.stage_b_magnitudes[kk];
      }
    } else if (checksum_index < valid_cols) {
#pragma unroll
      for (int kk = 0; kk < kK; ++kk) {
        float bvalue =
            static_cast<float>(sB_stage(checksum_index, kk));
        expected_checksum += storage.stage_a_sums[kk] * bvalue;
        checksum_scale +=
            storage.stage_a_magnitudes[kk] * fabsf(bvalue);
      }
    }
    __syncthreads();
...
#pragma unroll
    ...
    if (fabsf(delta) >
        abs_tolerance + rel_tolerance * fmaxf(1.0f, checksum_scale)) {
...
int main(int argc, char** argv) {
  ...
  auto kernel =
      wgmma_tma_cached_abft_kernel<decltype(tma_a), decltype(tma_b)>;
  ...
  auto enqueue_gemm = [&]() {
    ...
    CUDA_CHECK(cudaLaunchKernelEx(
        &config, kernel,
        tma_a, tma_b, d_a_sums, d_a_magnitudes, d_b_sums, d_b_magnitudes,
        d_c, d_bad_tiles, d_corrected_tiles, options.m, options.n, options.k,
        tile_rows, tile_cols, options.abft_abs_tolerance,
  ...
  size_t metadata_bytes =
      2 * (a_metadata_count + b_metadata_count) * sizeof(float);
