...
#pragma unroll
  ...
  if (threadIdx.x < kAbftSubtiles * 32) {
    ...
    if (valid_cols > 0) {
      ...
      for (int checksum_index = lane; checksum_index < kM; checksum_index += 32) {
...
#pragma unroll
        ...
        if (fabsf(row_delta) > abs_tolerance + rel_tolerance * fmaxf(1.0f, row_scales[micro_tile_id * kM + checksum_index])) {
          atomicAdd(&storage.bad_row_count[subtile], 1);
          atomicCAS(&storage.bad_row[subtile], -1, checksum_index);
        }
...
#pragma unroll
          ...
          if (fabsf(col_delta) > abs_tolerance + rel_tolerance * fmaxf(1.0f, col_scales[micro_tile_id * kN + checksum_index])) {
            ...
            atomicCAS(&storage.bad_col[subtile], -1, checksum_index);
          }
        }
      ...
      if (lane == 0) {
        if (storage.bad_row_count[subtile] > 0 || storage.bad_col_count[subtile] > 0) {
  ...
  for (int index = threadIdx.x; index < kCtaM * kCtaN; index += kThreads) {
    int row = index % kCtaM;
    int col = index / kCtaM;
...
int main(int argc, char** argv) {
  ...
  CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&active_blocks_per_sm, kernel, kThreads, sizeof(PipelineSharedStorage)));
  ...
  std::printf("version: S4 K128 2x1 cluster-multicast TMA-WGMMA ABFT\n");
  std::printf(
