...
template <int HeadDim>
__global__ __launch_bounds__(kThreads) void ft_flash_attention_a6_kernel(
  ...
  for (int key_begin = 0; key_begin < sequence; key_begin += kKeyTile) {
    ...
    if constexpr (HeadDim == 64) {
      int output_dim = tid >> 1;
      int key_parity = tid & 1;
      float partial = 0.0f;
      for (int local_key = key_parity; local_key < valid_keys; local_key += 2) {
        int key_index = key_begin + local_key;
        float vvalue = static_cast<float>(
            v[qkv_offset(batch_id, head, key_index, output_dim, heads, sequence, HeadDim)]);
        partial += exp_scores[local_key] * vvalue;
      }
      partial += __shfl_xor_sync(0xffffffffU, partial, 1);
      if (key_parity == 0) {
        output_accumulator = stage_alpha * output_accumulator + partial;
      }
    } else if (tid < HeadDim) {
  ...
  if constexpr (HeadDim == 64) {
    if ((tid & 1) == 0) {
      int output_dim = tid >> 1;
      output_shared[output_dim] = output_accumulator / online_l;
      if (fault_stage == kFaultOutput && target_row && output_dim == fault_index) {
        output_shared[output_dim] += fault_value;
      }
    }
  } else if (tid < HeadDim) {
...
int main(int argc, char** argv) {
  ...
  auto launch = [&](FaultStage fault_stage, cudaStream_t stream) {
    if (options.head_dim == 64) {
      ft_flash_attention_a6_kernel<64><<<grid, kThreads, 0, stream>>>(
      ...
      ft_flash_attention_a6_kernel<128><<<grid, kThreads, 0, stream>>>(
          d_q, d_k, d_v, d_output, options.batch, options.heads,
          d_k_sum, d_k_weighted, d_k_abs_sum, d_k_weighted_abs_sum, d_v_sum, d_v_weighted,
          options.sequence, key_tiles, options.causal, score_scale, static_cast<int>(fault_stage), options.fault_batch, options.fault_head,
          options.fault_query, options.fault_index, options.fault_value, options.score_abs_tolerance,
  ...
  std::printf("version: FT-FlashAttention A6 cooperative QK/PV dot products\n");
