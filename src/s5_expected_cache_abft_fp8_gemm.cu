...
constexpr uint32_t kTmaTransactionBytes = (kM * kK + kN * kK) * sizeof(Element);
...
static Options parse_options(int argc, char** argv) {
  ...
  if (options.m <= 0 || options.n <= 0 || options.k <= 0 || options.m % kM != 0 || options.k % kK != 0 || options.warmup < 0 ||
    ...
    std::fprintf(stderr, "Invalid options. Require M%%64==0, K%%32==0, and positive shape.\n");
    std::exit(EXIT_FAILURE);
  }
  return options;
}
...
__global__ void cache_expected_tile_checksums_kernel(const Element* a, const Element* b, const float* a_sums, const float* a_magnitudes,
                                                     const float* b_sums, const float* b_magnitudes, float* expected_rows,
                                                     float* expected_cols, float* row_scales, float* col_scales, int n, int k,
...
#pragma unroll
  ...
  if (lane == 0) {
    if (is_row) {
      ...
      size_t output_index = static_cast<size_t>(tile_id) * kN + row_or_col;
...
__global__ void __cluster_dims__(1, 1, 1)
    wgmma_tma_expected_cache_abft_kernel(CUTLASS_GRID_CONSTANT TmaA const tma_a, CUTLASS_GRID_CONSTANT TmaB const tma_b,
                                         ...
                                         int tile_rows, int tile_cols, float abs_tolerance, float rel_tolerance, float fault_value,
                                         bool inject_fault) {
  extern __shared__ __align__(128) unsigned char shared_bytes[];
  PipelineSharedStorage& storage = *reinterpret_cast<PipelineSharedStorage*>(shared_bytes);

  int local_thread = threadIdx.x < kComputeThreads ? threadIdx.x : threadIdx.x - kComputeThreads;
...
#pragma unroll
    for (int stage = 0; stage < kOutputStages; ++stage) {
      cutlass::arch::ClusterBarrier::init(&storage.output_full_barrier[stage], 1);
      cutlass::arch::ClusterBarrier::init(&storage.output_empty_barrier[stage], 1);
    }
...
#pragma unroll
    for (int stage = 0; stage < kOutputStages; ++stage) {
      cutlass::arch::ClusterBarrier::arrive(&storage.output_empty_barrier[stage]);
    }
  }
  __syncthreads();
  ...
  if (is_compute_group) {
    ...
    bool is_tma_leader = local_thread < 32 && static_cast<bool>(cute::elect_one_sync());
    if (is_tma_leader) {
      cute::prefetch_tma_descriptor(tma_a.get_tma_descriptor());
      cute::prefetch_tma_descriptor(tma_b.get_tma_descriptor());
    }

    auto sA = cute::make_tensor(cute::make_smem_ptr(storage.a), SmemLayoutAStages{});
    auto sB = cute::make_tensor(cute::make_smem_ptr(storage.b), SmemLayoutBStages{});
    using CLayout = decltype(cute::make_layout(cute::make_shape(cute::Int<kM>{}, cute::Int<kN>{}),
    ...
    for (int task = 0, tile_id = blockIdx.x; tile_id < tile_count; ++task, tile_id += gridDim.x) {
      int output_stage = task % kOutputStages;
      uint32_t output_phase = (task / kOutputStages) & 1;
      cutlass::arch::ClusterBarrier::wait(&storage.output_empty_barrier[output_stage], output_phase);
...
#pragma unroll
      ...
      auto gA_tile = gA(cute::_, cute::_, tile_row, cute::_, cute::Int<0>{});
      auto gB_tile = gB(cute::_, cute::_, tile_col, cute::_, cute::Int<0>{});
      ...
      if (is_tma_leader) {
        cutlass::arch::ClusterTransactionBarrier::arrive_and_expect_tx(&storage.tma_full_barrier[0], kTmaTransactionBytes);
        cute::copy(tma_a.with(storage.tma_full_barrier[0]), tAgA(cute::_, cute::_, cute::_, 0), tAsA(cute::_, cute::_, cute::_, 0));
        cute::copy(tma_b.with(storage.tma_full_barrier[0]), tBgB(cute::_, cute::_, cute::_, 0), tBsB(cute::_, cute::_, cute::_, 0));
      }
      ...
      for (int k_tile = 0; k_tile < k_tile_count; ++k_tile) {
        ...
        cutlass::arch::ClusterTransactionBarrier::wait(&storage.tma_full_barrier[read_stage], read_phase);

        int next_k_tile = k_tile + 1;
        if (next_k_tile < k_tile_count && is_tma_leader) {
          int write_stage = next_k_tile % kStages;
          cutlass::arch::ClusterTransactionBarrier::arrive_and_expect_tx(&storage.tma_full_barrier[write_stage], kTmaTransactionBytes);
          cute::copy(tma_a.with(storage.tma_full_barrier[write_stage]), tAgA(cute::_, cute::_, cute::_, next_k_tile),
                     tAsA(cute::_, cute::_, cute::_, write_stage));
          cute::copy(tma_b.with(storage.tma_full_barrier[write_stage]), tBgB(cute::_, cute::_, cute::_, next_k_tile),
      ...
      auto sC = cute::make_tensor(cute::make_smem_ptr(storage.c[output_stage]), CLayout{});
      ...
      cutlass::arch::NamedBarrier::sync(kComputeThreads, 0);
      if (is_group_leader) {
        cutlass::arch::ClusterBarrier::arrive(&storage.output_full_barrier[output_stage]);
      }
    }
  } else {
    ...
    for (int task = 0, tile_id = blockIdx.x; tile_id < tile_count; ++task, tile_id += gridDim.x) {
      int output_stage = task % kOutputStages;
      uint32_t output_phase = (task / kOutputStages) & 1;
      cutlass::arch::ClusterBarrier::wait(&storage.output_full_barrier[output_stage], output_phase);
      ...
      if (is_group_leader) {
        ...
        if (inject_fault && tile_id == tile_count / 2) {
          ...
          if (col >= 0) {
            storage.c[output_stage][col * kCStride + row] += fault_value;
          }
        }
      }
      cutlass::arch::NamedBarrier::sync(kVerifyThreads, 1);

      for (int checksum_index = local_thread; checksum_index < kM; checksum_index += kVerifyThreads) {
        float row_actual = 0.0f;
#pragma unroll
        for (int col = 0; col < kN; ++col) {
          if (col < valid_cols) {
            row_actual += storage.c[output_stage][col * kCStride + checksum_index];
          }
        }
        float row_delta = expected_rows[tile_id * kM + checksum_index] - row_actual;
        storage.deltas[output_stage][checksum_index] = row_delta;
        if (fabsf(row_delta) > abs_tolerance + rel_tolerance * fmaxf(1.0f, row_scales[tile_id * kM + checksum_index])) {
...
#pragma unroll
          for (int row = 0; row < kM; ++row) {
            col_actual += storage.c[output_stage][checksum_index * kCStride + row];
          }
          col_delta = expected_cols[tile_id * kN + checksum_index] - col_actual;
          if (fabsf(col_delta) > abs_tolerance + rel_tolerance * fmaxf(1.0f, col_scales[tile_id * kN + checksum_index])) {
            atomicAdd(&storage.bad_col_count[output_stage], 1);
            atomicCAS(&storage.bad_col[output_stage], -1, checksum_index);
          }
        }
        storage.deltas[output_stage][kM + checksum_index] = col_delta;
      }
      cutlass::arch::NamedBarrier::sync(kVerifyThreads, 1);

      if (is_group_leader) {
        if (storage.bad_row_count[output_stage] > 0 || storage.bad_col_count[output_stage] > 0) {
          atomicAdd(bad_tiles, 1);
        }
        if (storage.bad_row_count[output_stage] == 1 && storage.bad_col_count[output_stage] == 1) {
          int row = storage.bad_row[output_stage];
          int col = storage.bad_col[output_stage];
          storage.c[output_stage][col * kCStride + row] +=
              0.5f * (storage.deltas[output_stage][row] + storage.deltas[output_stage][kM + col]);
          atomicAdd(corrected_tiles, 1);
        }
      }
      cutlass::arch::NamedBarrier::sync(kVerifyThreads, 1);

      for (int index = local_thread; index < kM * kN; index += kVerifyThreads) {
        int row = index % kM;
        int col = index / kM;
        int global_col = tile_col * kN + col;
        if (global_col < n) {
          c[global_col * m + tile_row * kM + row] = storage.c[output_stage][col * kCStride + row];
        }
      }
      cutlass::arch::NamedBarrier::sync(kVerifyThreads, 1);
      if (is_group_leader) {
        cutlass::arch::ClusterBarrier::arrive(&storage.output_empty_barrier[output_stage]);
      }
    }
  }
}
...
static VerificationResult verify_samples(const std::vector<Element>& a, const std::vector<Element>& b, const std::vector<float>& c,
  ...
  for (int row = 0; row < options.m && checked_rows < options.verify_samples; row += row_step, ++checked_rows) {
    int checked_cols = 0;
    for (int col = 0; col < options.n && checked_cols < options.verify_samples; col += col_step, ++checked_cols) {
      double reference = 0.0;
      for (int kk = 0; kk < options.k; ++kk) {
        reference += static_cast<double>(static_cast<float>(a[row * options.k + kk])) *
                     static_cast<double>(static_cast<float>(b[col * options.k + kk]));
      }
      float absolute_error = static_cast<float>(std::fabs(c[col * options.m + row] - reference));
      float tolerance =
          options.verify_abs_tolerance + options.verify_rel_tolerance * std::max(1.0f, static_cast<float>(std::fabs(reference)));
...
int main(int argc, char** argv) {
  ...
  CUDA_CHECK(cudaMalloc(&d_expected_rows, row_metadata_count * sizeof(float)));
  CUDA_CHECK(cudaMalloc(&d_expected_cols, col_metadata_count * sizeof(float)));
  ...
  generate_fp8_kernel<<<static_cast<int>((a_count + generator_threads - 1) / generator_threads), generator_threads>>>(d_a, a_count,
                                                                                                                      0x12345678U);
  generate_fp8_kernel<<<static_cast<int>((b_count + generator_threads - 1) / generator_threads), generator_threads>>>(d_b, b_count,
                                                                                                                      0x9abcdef0U);
  ...
  auto kernel = wgmma_tma_expected_cache_abft_kernel<decltype(tma_a), decltype(tma_b)>;
  ...
  CUDA_CHECK(cudaDeviceGetAttribute(&multiprocessor_count, cudaDevAttrMultiProcessorCount, 0));
  CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&active_blocks_per_sm, kernel, kPipelineThreads, sizeof(PipelineSharedStorage)));
  int max_persistent_ctas = std::min(tile_count, multiprocessor_count * active_blocks_per_sm);
  int persistent_ctas = max_persistent_ctas;
  int tiles_per_persistent_cta = div_up(tile_count, persistent_ctas);
  ...
  int expected_metadata_blocks = div_up(tile_count * (kM + kN), metadata_warps);
  CUDA_CHECK(cudaEventRecord(start, stream));
  cache_expected_tile_checksums_kernel<<<expected_metadata_blocks, metadata_threads, 0, stream>>>(
      d_a, d_b, d_a_sums, d_a_magnitudes, d_b_sums, d_b_magnitudes, d_expected_rows, d_expected_cols, d_row_scales, d_col_scales, options.n,
      options.k, tile_rows, tile_cols);
  CUDA_CHECK(cudaEventRecord(stop, stream));
  CUDA_CHECK(cudaEventSynchronize(stop));
  CUDA_CHECK(cudaGetLastError());
  ...
  double first_call_tflops = operations / (first_call_total_ms * 1.0e9);
  ...
  std::printf("shared_c_stride: %d, persistent_ctas: %d, max_persistent_ctas: %d\n", kCStride, persistent_ctas, max_persistent_ctas);
  std::printf("tiles_per_persistent_cta: %d, active_blocks_per_sm: %d\n", tiles_per_persistent_cta, active_blocks_per_sm);
