...
struct AttentionTraits {
  ...
  using SmemLayoutKStages = decltype(cute::tile_to_shape(
      cute::SM90::GMMA::Layout_K_INTER_Atom<Element>{},
      cute::Shape<cute::_128, cute::Int<HeadDim>, cute::Int<kStages>>{},
      cute::Step<cute::_1, cute::_2, cute::_3>{}));
  using SmemLayoutV = decltype(cute::tile_to_shape(
      cute::SM90::GMMA::Layout_K_INTER_Atom<Element>{},
      cute::Shape<cute::Int<HeadDim>, cute::_128>{}));
  using SmemLayoutVStages = decltype(cute::tile_to_shape(
      cute::SM90::GMMA::Layout_K_INTER_Atom<Element>{},
      cute::Shape<cute::Int<HeadDim>, cute::_128, cute::Int<kStages>>{},
      cute::Step<cute::_1, cute::_2, cute::_3>{}));
  using QkTileShape =
      cute::Shape<cute::_64, cute::_128, cute::Int<HeadDim>>;
  using PvTileShape =
      cute::Shape<cute::_64, cute::Int<HeadDim>, cute::_128>;
};

template <int HeadDim>
struct alignas(128) AttentionSharedStorage {
  using Traits = AttentionTraits<HeadDim>;
  alignas(8) uint64_t q_full_barrier;
  alignas(8) uint64_t kv_full_barrier[kStages];
  alignas(8) uint64_t kv_empty_barrier[kStages];
  alignas(128) Element q[cute::cosize_v<typename Traits::SmemLayoutQ>];
  alignas(128) Element k[cute::cosize_v<typename Traits::SmemLayoutKStages>];
  alignas(128) Element v[cute::cosize_v<typename Traits::SmemLayoutVStages>];
};
...
__global__ void transpose_v_kernel(const Element* input, Element* output, int batch_heads,
                                   int sequence, int head_dim, int output_stride) {
  size_t index = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x;
  size_t count = static_cast<size_t>(batch_heads) * sequence * head_dim;
  if (index >= count) {
    return;
  }
  int dim = index % head_dim;
  size_t row = index / head_dim;
  int key = row % sequence;
  int batch_head = row / sequence;
  output[(static_cast<size_t>(batch_head) * head_dim + dim) * output_stride + key] =
      input[index];
}
...
template <class TmaQ, class TmaK, class TmaV>
struct A14Params {
  TmaQ tma_q;
  TmaK tma_k;
  TmaV tma_v;
  ...
  float* output;
  const float* k_sum;
  const float* k_weighted;
  const float* k_abs_sum;
  const float* k_weighted_abs_sum;
  const float* v_sum;
  ...
  float score_scale;
  float fault_value;
  float score_abs_tolerance;
  float score_rel_tolerance;
  float softmax_abs_tolerance;
  float softmax_rel_tolerance;
  float pv_abs_tolerance;
  float pv_rel_tolerance;
  bool causal;
};

template <int HeadDim, class Params>
__global__ __launch_bounds__(kThreads, 2) void ft_flash_attention_a14_kernel(
    CUTLASS_GRID_CONSTANT Params const params) {
  auto const& tma_q = params.tma_q;
  auto const& tma_k = params.tma_k;
  auto const& tma_v = params.tma_v;
  auto const* q = params.q;
  auto const* k = params.k;
  auto const* v = params.v;
  auto* output = params.output;
  auto const* k_sum = params.k_sum;
  auto const* k_weighted = params.k_weighted;
  auto const* k_abs_sum = params.k_abs_sum;
  auto const* k_weighted_abs_sum = params.k_weighted_abs_sum;
  auto const* v_sum = params.v_sum;
  auto const* v_weighted = params.v_weighted;
  auto* stats = params.stats;
  int batch = params.batch;
  int heads = params.heads;
  int sequence = params.sequence;
  int key_tiles = params.key_tiles;
  int fault_stage = params.fault_stage;
  int fault_batch = params.fault_batch;
  int fault_head = params.fault_head;
  int fault_query = params.fault_query;
  int fault_index = params.fault_index;
  float score_scale = params.score_scale;
  float fault_value = params.fault_value;
  float score_abs_tolerance = params.score_abs_tolerance;
  float score_rel_tolerance = params.score_rel_tolerance;
  float softmax_abs_tolerance = params.softmax_abs_tolerance;
  float softmax_rel_tolerance = params.softmax_rel_tolerance;
  float pv_abs_tolerance = params.pv_abs_tolerance;
  float pv_rel_tolerance = params.pv_rel_tolerance;
  bool causal = params.causal;
  using Traits = AttentionTraits<HeadDim>;
  ...
  auto sK = cute::make_tensor(cute::make_smem_ptr(storage.k),
                              typename Traits::SmemLayoutKStages{});
  auto sV = cute::make_tensor(cute::make_smem_ptr(storage.v),
                              typename Traits::SmemLayoutVStages{});

  int block_thread = threadIdx.x;
  int query_begin = blockIdx.x * kQueryTile;
  int batch_head = blockIdx.y;
  int batch_id = batch_head / heads;
  int head = batch_head % heads;
  int key_limit = causal ? min(sequence, query_begin + kQueryTile) : sequence;
  int key_tile_count = (key_limit + kKeyTile - 1) / kKeyTile;

  if (block_thread == 0) {
    cutlass::arch::ClusterTransactionBarrier::init(&storage.q_full_barrier, 1);
#pragma unroll
    for (int stage = 0; stage < kStages; ++stage) {
      cutlass::arch::ClusterTransactionBarrier::init(&storage.kv_full_barrier[stage], 1);
      cutlass::arch::ClusterBarrier::init(&storage.kv_empty_barrier[stage], 1);
    }
    cutlass::arch::fence_barrier_init();
  }
  __syncthreads();

  if (block_thread < kProducerThreads) {
...
#pragma unroll
      for (int stage = 0; stage < kStages; ++stage) {
        cutlass::arch::ClusterBarrier::arrive(&storage.kv_empty_barrier[stage]);
      }

      auto mQ =
          tma_q.get_tma_tensor(cute::make_shape(sequence, HeadDim, batch * heads));
      auto mK =
          tma_k.get_tma_tensor(cute::make_shape(sequence, HeadDim, batch * heads));
      auto mV =
          tma_v.get_tma_tensor(cute::make_shape(HeadDim, sequence, batch * heads));
      auto gQ = cute::local_tile(
          mQ, typename Traits::QkTileShape{}, cute::make_coord(cute::_, cute::_, cute::_),
          cute::Step<cute::_1, cute::X, cute::_1>{});
      auto gK = cute::local_tile(
          mK, typename Traits::QkTileShape{}, cute::make_coord(cute::_, cute::_, cute::_),
          cute::Step<cute::X, cute::_1, cute::_1>{});
      auto gV = cute::local_tile(
          mV, typename Traits::PvTileShape{}, cute::make_coord(cute::_, cute::_, cute::_),
          cute::Step<cute::X, cute::_1, cute::_1>{});
      auto tma_q_slice = tma_q.get_slice(0);
      auto tma_k_slice = tma_k.get_slice(0);
      auto tma_v_slice = tma_v.get_slice(0);
      auto q_source =
          tma_q_slice.partition_S(gQ(cute::_, cute::_, blockIdx.x, cute::Int<0>{},
                                    batch_head));
      auto q_destination = tma_q_slice.partition_D(sQ);
      cutlass::arch::ClusterTransactionBarrier::arrive_and_expect_tx(
          &storage.q_full_barrier, kQueryTile * HeadDim * sizeof(Element));
      cute::copy(tma_q.with(storage.q_full_barrier), q_source, q_destination);

      for (int key_tile = 0; key_tile < key_tile_count; ++key_tile) {
        int stage = key_tile % kStages;
        uint32_t phase = (key_tile / kStages) & 1;
        cutlass::arch::ClusterBarrier::wait(&storage.kv_empty_barrier[stage], phase);
        auto k_source = tma_k_slice.partition_S(
            gK(cute::_, cute::_, key_tile, cute::Int<0>{}, batch_head));
        auto k_destination = tma_k_slice.partition_D(sK(cute::_, cute::_, stage));
        auto v_source = tma_v_slice.partition_S(
            gV(cute::_, cute::_, cute::Int<0>{}, key_tile, batch_head));
        auto v_destination = tma_v_slice.partition_D(sV(cute::_, cute::_, stage));
        cutlass::arch::ClusterTransactionBarrier::arrive_and_expect_tx(
            &storage.kv_full_barrier[stage],
            2 * kKeyTile * HeadDim * sizeof(Element));
        cute::copy(tma_k.with(storage.kv_full_barrier[stage]), k_source, k_destination);
        cute::copy(tma_v.with(storage.kv_full_barrier[stage]), v_source, v_destination);
      }
    }
  } else {
  cutlass::arch::warpgroup_reg_alloc<192>();
  int tid = block_thread - kProducerThreads;
  int quad_lane = tid & 3;
  cutlass::arch::ClusterTransactionBarrier::wait(&storage.q_full_barrier, 0);
  QkTiledMma qk_mma;
...
#pragma unroll
  ...
  for (int key_begin = 0; key_begin < key_limit; key_begin += kKeyTile) {
    ...
    cutlass::arch::ClusterTransactionBarrier::wait(
        &storage.kv_full_barrier[stage], phase);
    auto sK_stage = sK(cute::_, cute::_, stage);
    auto sV_stage = sV(cute::_, cute::_, stage);
    auto k_descriptor = qk_thread.partition_fragment_B(sK_stage);
    auto v_descriptor = pv_thread.partition_fragment_B(sV_stage);
      ...
      if (valid_keys > 0) {
        ...
        for (int dim = quad_lane; dim < HeadDim; dim += 4) {
          ...
          if (valid_keys == full_tile_keys) {
            ...
            for (int local_key = 0; local_key < valid_keys; ++local_key) {
              float key_value = static_cast<float>(sK_stage(local_key, dim));
            ...
            if (query < sequence && key_index < sequence && (!causal || key_index <= query)) {
              ...
              for (int dim = 0; dim < HeadDim; ++dim) {
                replay += static_cast<float>(sQ(row, dim)) *
                          static_cast<float>(sK_stage(local_key, dim));
              }
...
template <int HeadDim>
static float run_attention_a14(
    const Options& options, Element* d_q, Element* d_k, Element* d_v,
    Element* d_v_transposed, float* d_output, DeviceStats* d_stats, float* d_k_sum,
    float* d_k_weighted, float* d_k_abs_sum, float* d_k_weighted_abs_sum, float* d_v_sum,
    float* d_v_weighted, int v_transposed_stride, int key_tiles, dim3 grid,
    cudaStream_t stream, cudaEvent_t start, cudaEvent_t stop) {
  using Traits = AttentionTraits<HeadDim>;
  auto tensor_q = cute::make_tensor(
      d_q,
      cute::make_layout(
          cute::make_shape(options.sequence, HeadDim, options.batch * options.heads),
          cute::make_stride(HeadDim, cute::_1{},
                            static_cast<int64_t>(options.sequence) * HeadDim)));
  auto tensor_k = cute::make_tensor(
      d_k,
      cute::make_layout(
          cute::make_shape(options.sequence, HeadDim, options.batch * options.heads),
          cute::make_stride(HeadDim, cute::_1{},
                            static_cast<int64_t>(options.sequence) * HeadDim)));
  auto tensor_v = cute::make_tensor(
      d_v_transposed,
      cute::make_layout(
          cute::make_shape(HeadDim, options.sequence, options.batch * options.heads),
          cute::make_stride(v_transposed_stride, cute::_1{},
                            static_cast<int64_t>(v_transposed_stride) * HeadDim)));
  auto tma_q = cute::make_tma_copy_A_sm90(
      cute::SM90_TMA_LOAD{}, tensor_q, typename Traits::SmemLayoutQ{},
      typename Traits::QkTileShape{}, ClusterShape{});
  auto tma_k = cute::make_tma_copy_B_sm90(
      cute::SM90_TMA_LOAD{}, tensor_k,
      typename Traits::SmemLayoutKStages{}(cute::_, cute::_, cute::Int<0>{}),
      typename Traits::QkTileShape{}, ClusterShape{});
  auto tma_v = cute::make_tma_copy_B_sm90(
      cute::SM90_TMA_LOAD{}, tensor_v,
      typename Traits::SmemLayoutVStages{}(cute::_, cute::_, cute::Int<0>{}),
      typename Traits::PvTileShape{}, ClusterShape{});
  using Params = A14Params<decltype(tma_q), decltype(tma_k), decltype(tma_v)>;
  auto kernel = ft_flash_attention_a14_kernel<HeadDim, Params>;
  CUDA_CHECK(cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize,
                                  sizeof(AttentionSharedStorage<HeadDim>)));
  float score_scale = 1.0f / std::sqrt(static_cast<float>(HeadDim));
  auto launch = [&](FaultStage fault_stage) {
    Params params{
        tma_q, tma_k, tma_v, d_q, d_k, d_v, d_output,
        d_k_sum, d_k_weighted, d_k_abs_sum, d_k_weighted_abs_sum, d_v_sum, d_v_weighted,
        d_stats, options.batch, options.heads, options.sequence, key_tiles,
        static_cast<int>(fault_stage), options.fault_batch, options.fault_head,
        options.fault_query, options.fault_index, score_scale, options.fault_value,
        options.score_abs_tolerance, options.score_rel_tolerance,
        options.softmax_abs_tolerance, options.softmax_rel_tolerance,
        options.pv_abs_tolerance, options.pv_rel_tolerance, options.causal};
    kernel<<<grid, kThreads, sizeof(AttentionSharedStorage<HeadDim>), stream>>>(params);
  };
  ...
  CUDA_CHECK(cudaMemsetAsync(d_stats, 0, sizeof(DeviceStats), stream));
  launch(options.fault_stage);
  CUDA_CHECK(cudaStreamSynchronize(stream));
  CUDA_CHECK(cudaGetLastError());
  return total_ms / options.repeat;
}
...
int main(int argc, char** argv) {
  ...
  size_t element_count = static_cast<size_t>(options.batch) * options.heads * options.sequence *
                         options.head_dim;
  int v_transposed_stride = (options.sequence + 15) / 16 * 16;
  size_t v_transposed_count = static_cast<size_t>(options.batch) * options.heads *
                              options.head_dim * v_transposed_stride;
  Element* d_q = nullptr;
  Element* d_k = nullptr;
  Element* d_v = nullptr;
  Element* d_v_transposed = nullptr;
  ...
  CUDA_CHECK(cudaMalloc(&d_v_transposed, v_transposed_count * sizeof(Element)));
  ...
  transpose_v_kernel<<<generation_blocks, generation_threads>>>(
      d_v, d_v_transposed, options.batch * options.heads, options.sequence,
      options.head_dim, v_transposed_stride);
  ...
  float average_ms = options.head_dim == 64
      ? run_attention_a14<64>(options, d_q, d_k, d_v, d_v_transposed, d_output, d_stats,
                              d_k_sum, d_k_weighted, d_k_abs_sum,
                              d_k_weighted_abs_sum, d_v_sum, d_v_weighted,
                              v_transposed_stride, key_tiles, grid, stream, start, stop)
      : run_attention_a14<128>(options, d_q, d_k, d_v, d_v_transposed, d_output, d_stats,
                               d_k_sum, d_k_weighted, d_k_abs_sum,
                               d_k_weighted_abs_sum, d_v_sum, d_v_weighted,
                               v_transposed_stride, key_tiles, grid, stream, start, stop);
  ...
  std::printf("version: FT-FlashAttention A14 register-resident TMA warp specialization\n");
  ...
  CUDA_CHECK(cudaFree(d_v_transposed));
