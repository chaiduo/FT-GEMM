...
constexpr uint32_t kTmaTransactionBytes =
    (kCtaM * kK + kCtaN * kK) * sizeof(Element);
...
static Options parse_options(int argc, char** argv) {
  ...
  if (options.m <= 0 || options.n <= 0 || options.k <= 0 ||
      options.m % kCtaM != 0 || options.k % kK != 0 ||
    ...
    std::fprintf(
        stderr,
        "Invalid options. Require M%%128==0, K%%32==0, and positive shape.\n");
    std::exit(EXIT_FAILURE);
  }
  return options;
}
...
wgmma_tma_wide_warpspecialized_abft_kernel(
...
#pragma unroll
    for (int stage = 0; stage < kStages; ++stage) {
      ...
      cutlass::arch::ClusterBarrier::init(
          &storage.tma_empty_barrier[stage], kConsumerWarpgroups);
    }
...
#pragma unroll
  ...
  if (is_producer) {
    auto sA0 = cute::make_tensor(
        cute::make_smem_ptr(storage.a[0]), SmemLayoutAStages{});
    auto sA1 = cute::make_tensor(
        cute::make_smem_ptr(storage.a[1]), SmemLayoutAStages{});
    ...
    if (is_producer_leader) {
      for (int k_tile = 0; k_tile < k_tile_count; ++k_tile) {
        ...
        cute::copy(
            tma_a.with(storage.tma_full_barrier[stage]),
            tAgA0(cute::_, cute::_, cute::_, k_tile),
            tAsA0(cute::_, cute::_, cute::_, stage));
        cute::copy(
            tma_a.with(storage.tma_full_barrier[stage]),
            tAgA1(cute::_, cute::_, cute::_, k_tile),
            tAsA1(cute::_, cute::_, cute::_, stage));
        cute::copy(
            tma_b.with(storage.tma_full_barrier[stage]),
            tBgB(cute::_, cute::_, cute::_, k_tile),
    ...
    auto sA = cute::make_tensor(
        cute::make_smem_ptr(storage.a[consumer_group]),
        SmemLayoutAStages{});
    auto sB = cute::make_tensor(
        cute::make_smem_ptr(storage.b), SmemLayoutBStages{});
    TiledMma tiled_mma;
    ...
    auto sC = cute::make_tensor(
        cute::make_smem_ptr(storage.c[consumer_group]), CLayout{});
  ...
  for (int index = threadIdx.x;
    ...
    if (global_row < m && global_col < n) {
      ...
      c[global_col * m + global_row] =
          storage.c[row_group][col * kCStride + local_row];
    }
  }
}
...
int main(int argc, char** argv) {
  ...
  size_t metadata_bytes =
      ...
      sizeof(float);
  std::printf(
      "version: S6 128x128 warp-specialized TMA-WGMMA ABFT\n");
  std::printf(
      "instruction_tile: 64x128x32, cta_tile: %dx%dx%d, stages: %d\n",
