...
static Options parse_options(int argc, char** argv) {
  ...
  options.abs_tolerance = get_float(argc, argv, "--checksum-tol", options.abs_tolerance);
  options.rel_tolerance = get_float(argc, argv, "--rel-tol", options.rel_tolerance);
  options.random_abs_tolerance = get_float(argc, argv, "--random-abs-tol", options.random_abs_tolerance);
  options.random_rel_tolerance = get_float(argc, argv, "--random-rel-tol", options.random_rel_tolerance);
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (std::strcmp(argv[i], "--random-abs-tol") == 0 || std::strncmp(argv[i], "--random-abs-tol=", 17) == 0) {
  ...
  if (std::max(options.m, std::max(options.n, options.k)) > 1024 && !options.random_abs_tolerance_explicit) {
    options.random_abs_tolerance = 4.8e1f;
  }
  if (options.m <= 0 || options.n <= 0 || options.k <= 0 || options.m % kM != 0 || options.k % kK != 0 || options.warmup < 0 ||
      options.repeat <= 0 || options.abs_tolerance < 0.0f || options.rel_tolerance <= 0.0f || options.random_abs_tolerance < 0.0f ||
      options.random_rel_tolerance <= 0.0f || options.batch_tiles <= 0) {
    std::fprintf(stderr, "Invalid shape/options. Require M%%64==0 and K%%32==0.\n");
    std::exit(EXIT_FAILURE);
  }
  return options;
}
...
__global__ void generate_sign_weights_kernel(float* output, size_t count, uint32_t seed) {
  size_t index = static_cast<size_t>(blockIdx.x) * blockDim.x + threadIdx.x;
  if (index < count) {
    uint32_t bits = mix_u32(seed + static_cast<uint32_t>(index));
    output[index] = (bits & 3U) == 0U ? ((bits & 4U) ? 1.0f : -1.0f) : 0.0f;
  }
}

__global__ void zero_b_padding_kernel(Element* b, int k, int n, int n_padded) {
...
__global__ void wgmma_gemm_kernel(const Element* a, const Element* b, float* c, float* actual_rows, float* actual_cols,
                                  float* actual_random_rows, float* actual_random_cols, const float* row_weights, const float* col_weights,
                                  int m, int n, int k, int tile_rows, int tile_cols, int tile_begin, int tile_count, bool inject_fault) {
  ...
  auto accum = cute::partition_fragment_C(tiled_mma, cute::make_shape(cute::_64{}, cute::_64{}));
  cute::clear(accum);
  ...
  for (int k_base = 0; k_base < k; k_base += kK) {
    for (int index = threadIdx.x; index < kM * kK; index += kThreads) {
      int row = index / kK;
      int kk = index % kK;
      sA(row, kk) = a[(tile_row * kM + row) * k + k_base + kk];
    }
    for (int index = threadIdx.x; index < kN * kK; index += kThreads) {
      int col = index / kK;
      int kk = index % kK;
      int global_col = tile_col * kN + col;
      sB(col, kk) = global_col < n ? b[global_col * k + k_base + kk] : Element(0.0f);
    }
    __syncthreads();
    ...
    tiled_mma.accumulate_ = k_base == 0 ? cute::GMMA::ScaleOut::Zero : cute::GMMA::ScaleOut::One;
  ...
  if (inject_fault && tile_id == (tile_rows * tile_cols) / 2 && threadIdx.x == 0) {
    int row = kM / 2;
    int col = kN / 2;
    if (actual_random_rows != nullptr && actual_random_cols != nullptr) {
      ...
      for (int candidate = 0; candidate < kN; ++candidate) {
        if (tile_col * kN + candidate < n && col_weights[tile_col * kN + candidate] != 0.0f) {
          col = candidate;
          break;
        }
      }
  ...
  if (actual_rows != nullptr && threadIdx.x < kM) {
    ...
    if (actual_random_rows != nullptr) {
      ...
      for (int col = 0; col < valid_cols; ++col) {
        random_actual += c_smem[col * kM + threadIdx.x] * col_weights[tile_col * kN + col];
      }
  ...
  if (actual_cols != nullptr && threadIdx.x < kN) {
    ...
    if (actual_random_cols != nullptr) {
      ...
      if (threadIdx.x < valid_cols) {
        for (int row = 0; row < kM; ++row) {
          random_actual += c_smem[threadIdx.x * kM + row] * row_weights[tile_row * kM + row];
        }
      }
...
__global__ void input_tile_sums_kernel(const Element* a, const Element* b, float* a_sums, float* a_magnitudes, float* b_sums,
                                       float* b_magnitudes, float* a_weighted_sums, float* b_weighted_sums, const float* row_weights,
                                       const float* col_weights, int m, int n, int n_padded, int k, int tile_rows, int tile_cols) {
  ...
  if (tile_id < tile_rows) {
    for (int kk = threadIdx.x; kk < k; kk += blockDim.x) {
      ...
      for (int row = 0; row < kM; ++row) {
        float value = static_cast<float>(a[(tile_id * kM + row) * k + kk]);
        sum += value;
  ...
  if (tile_id < tile_cols) {
    ...
    for (int kk = threadIdx.x; kk < k; kk += blockDim.x) {
      ...
      float weighted_sum = 0.0f;
      for (int col = 0; col < valid_cols; ++col) {
        float value = static_cast<float>(b[(tile_id * kN + col) * k + kk]);
        sum += value;
        magnitude += fabsf(value);
...
__global__ void expected_tile_checksums_kernel(const Element* a, const Element* b, const float* a_sums, const float* a_magnitudes,
                                               const float* b_sums, const float* b_magnitudes, const float* a_weighted_sums,
                                               const float* b_weighted_sums, float* expected_rows, float* expected_cols,
                                               float* expected_random_rows, float* expected_random_cols, float* row_scales,
                                               float* col_scales, int m, int n, int k, int tile_rows, int tile_cols) {
  int tile_id = blockIdx.x;
...
__global__ void tile_checksum_kernel(const float* actual_rows, const float* actual_cols, const float* actual_random_rows,
                                     const float* actual_random_cols, const float* expected_rows, const float* expected_cols,
                                     const float* expected_random_rows, const float* expected_random_cols, float* row_deltas,
                                     float* col_deltas, float* random_row_deltas, float* random_col_deltas, int n, int tile_rows,
                                     int tile_cols, int tile_begin, int tile_count) {
  int tile_id = tile_begin + blockIdx.x;
  int tile_col = tile_id % tile_cols;
  if (blockIdx.x >= tile_count || tile_id >= tile_rows * tile_cols) {
    return;
  ...
  int valid_cols = min(kN, n - tile_col * kN);
  if (threadIdx.x < kM) {
    row_deltas[tile_id * kM + threadIdx.x] = expected_rows[tile_id * kM + threadIdx.x] - actual_rows[tile_id * kM + threadIdx.x];
    random_row_deltas[tile_id * kM + threadIdx.x] =
        expected_random_rows[tile_id * kM + threadIdx.x] - actual_random_rows[tile_id * kM + threadIdx.x];
  }
  if (threadIdx.x < kN) {
    col_deltas[tile_id * kN + threadIdx.x] =
        threadIdx.x < valid_cols ? expected_cols[tile_id * kN + threadIdx.x] - actual_cols[tile_id * kN + threadIdx.x] : 0.0f;
    random_col_deltas[tile_id * kN + threadIdx.x] =
        threadIdx.x < valid_cols ? expected_random_cols[tile_id * kN + threadIdx.x] - actual_random_cols[tile_id * kN + threadIdx.x] : 0.0f;
  }
}
...
__global__ void tile_abft_check_kernel(float* c, const float* row_deltas, const float* col_deltas, const float* random_row_deltas,
                                       const float* random_col_deltas, const float* row_scales, const float* col_scales, int m, int n,
                                       int tile_rows, int tile_cols, int tile_begin, int tile_count, float abs_tolerance,
                                       float rel_tolerance, float random_abs_tolerance, float random_rel_tolerance, int* bad_tiles,
                                       int* corrected_tiles, float* max_normalized_error, float* max_absolute_error,
                                       float* max_random_absolute_error) {
  int tile_id = tile_begin + blockIdx.x;
  int tile_row = tile_id / tile_cols;
  int tile_col = tile_id % tile_cols;
  if (blockIdx.x >= tile_count || tile_id >= tile_rows * tile_cols) {
  ...
  for (int row = threadIdx.x; row < kM; row += kThreads) {
    ...
    bool bad = diff > abs_tolerance + rel_tolerance * scale || random_diff > random_abs_tolerance + random_rel_tolerance * scale;
    if (bad) {
...
static float max_reference_error(const std::vector<Element>& a, const std::vector<Element>& b, const std::vector<float>& c, int m, int n,
                                 int k) {
  float maximum = 0.0f;
  for (int row = 0; row < m; row += std::max(1, m / 8)) {
    for (int col = 0; col < n; col += std::max(1, n / 8)) {
      ...
      for (int kk = 0; kk < k; ++kk) {
        reference += static_cast<double>(static_cast<float>(a[row * k + kk])) * static_cast<double>(static_cast<float>(b[col * k + kk]));
      }
      maximum = std::max(maximum, static_cast<float>(std::fabs(c[col * m + row] - reference)));
    }
  }
  return maximum;
}
...
int main(int argc, char** argv) {
  ...
  CUDA_CHECK(cudaMalloc(&d_a_magnitudes, tile_rows * options.k * sizeof(float)));
  CUDA_CHECK(cudaMalloc(&d_b_sums, tile_cols * options.k * sizeof(float)));
  CUDA_CHECK(cudaMalloc(&d_b_magnitudes, tile_cols * options.k * sizeof(float)));
  CUDA_CHECK(cudaMalloc(&d_a_weighted_sums, tile_rows * options.k * sizeof(float)));
  CUDA_CHECK(cudaMalloc(&d_b_weighted_sums, tile_cols * options.k * sizeof(float)));
  ...
  generate_sign_weights_kernel<<<(options.m + threads - 1) / threads, threads>>>(d_row_weights, options.m, 0x13579bdfU);
  generate_sign_weights_kernel<<<(n_padded + threads - 1) / threads, threads>>>(d_col_weights, n_padded, 0x2468ace0U);
  CUDA_CHECK(cudaGetLastError());
  CUDA_CHECK(cudaDeviceSynchronize());
  if (n_padded > options.n) {
    int padding_count = (n_padded - options.n) * options.k;
    zero_b_padding_kernel<<<(padding_count + threads - 1) / threads, threads>>>(d_b, options.k, options.n, n_padded);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
  }
  if (options.no_abft) {
    ...
    CUDA_CHECK(cudaStreamCreateWithFlags(&baseline_stream, cudaStreamNonBlocking));
    CUDA_CHECK(cudaEventCreate(&baseline_start));
    CUDA_CHECK(cudaEventCreate(&baseline_stop));
    auto enqueue_gemm = [&]() {
      wgmma_gemm_kernel<<<tile_count, kThreads, kGemmSmemBytes, baseline_stream>>>(d_a, d_b, d_c, nullptr, nullptr, nullptr, nullptr,
                                                                                   nullptr, nullptr, options.m, options.n, options.k,
    ...
    for (int i = 0; i < options.repeat; ++i) {
      ...
      CUDA_CHECK(cudaEventElapsedTime(&iteration_ms, baseline_start, baseline_stop));
  ...
  input_tile_sums_kernel<<<std::max(tile_rows, tile_cols), kThreads, 0, checksum_stream>>>(
      d_a, d_b, d_a_sums, d_a_magnitudes, d_b_sums, d_b_magnitudes, d_a_weighted_sums, d_b_weighted_sums, d_row_weights, d_col_weights,
      options.m, options.n, n_padded, options.k, tile_rows, tile_cols);
  expected_tile_checksums_kernel<<<tile_count, kThreads, 0, checksum_stream>>>(
      d_a, d_b, d_a_sums, d_a_magnitudes, d_b_sums, d_b_magnitudes, d_a_weighted_sums, d_b_weighted_sums, d_expected_rows, d_expected_cols,
      d_expected_random_rows, d_expected_random_cols, d_row_scales, d_col_scales, options.m, options.n, options.k, tile_rows, tile_cols);
  ...
  auto enqueue_iteration = [&]() {
    for (int batch = 0; batch < batch_count; ++batch) {
      ...
      wgmma_gemm_kernel<<<tiles_this_batch, kThreads, kGemmSmemBytes, gemm_stream>>>(
          d_a, d_b, d_c, d_row_deltas, d_col_deltas, d_random_row_deltas, d_random_col_deltas, d_row_weights, d_col_weights, options.m,
          options.n, options.k, tile_rows, tile_cols, tile_begin, tiles_this_batch, options.inject_fault);
      ...
      tile_checksum_kernel<<<tiles_this_batch, kThreads, 0, checksum_stream>>>(
          d_row_deltas, d_col_deltas, d_random_row_deltas, d_random_col_deltas, d_expected_rows, d_expected_cols, d_expected_random_rows,
          d_expected_random_cols, d_row_deltas, d_col_deltas, d_random_row_deltas, d_random_col_deltas, options.n, tile_rows, tile_cols,
      ...
      if (batch == 0) {
        ...
        CUDA_CHECK(cudaMemsetAsync(d_max_normalized_error, 0, sizeof(float), abft_stream));
        CUDA_CHECK(cudaMemsetAsync(d_max_absolute_error, 0, sizeof(float), abft_stream));
        CUDA_CHECK(cudaMemsetAsync(d_max_random_absolute_error, 0, sizeof(float), abft_stream));
      }
      CUDA_CHECK(cudaEventRecord(abft_start[batch], abft_stream));
      tile_abft_check_kernel<<<tiles_this_batch, kThreads, 0, abft_stream>>>(
          d_c, d_row_deltas, d_col_deltas, d_random_row_deltas, d_random_col_deltas, d_row_scales, d_col_scales, options.m, options.n,
          tile_rows, tile_cols, tile_begin, tiles_this_batch, options.abs_tolerance, options.rel_tolerance, options.random_abs_tolerance,
          options.random_rel_tolerance, d_bad_tiles, d_corrected_tiles, d_max_normalized_error, d_max_absolute_error,
          d_max_random_absolute_error);
  ...
  for (int i = 0; i < options.repeat; ++i) {
    ...
    for (int batch = 0; batch < batch_count; ++batch) {
      ...
      CUDA_CHECK(cudaEventElapsedTime(&batch_gemm_ms, gemm_start[batch], gemm_done[batch]));
      CUDA_CHECK(cudaEventElapsedTime(&batch_checksum_ms, checksum_start[batch], checksum_done[batch]));
      CUDA_CHECK(cudaEventElapsedTime(&batch_abft_ms, abft_start[batch], abft_done[batch]));
  ...
  CUDA_CHECK(cudaEventElapsedTime(&metadata_ms, metadata_start, metadata_done));
  ...
  float error = max_reference_error(h_a, h_b, h_c, options.m, options.n, options.k);
  ...
  CUDA_CHECK(cudaMemcpy(&max_normalized_error, d_max_normalized_error, sizeof(float), cudaMemcpyDeviceToHost));
  CUDA_CHECK(cudaMemcpy(&max_absolute_error, d_max_absolute_error, sizeof(float), cudaMemcpyDeviceToHost));
  CUDA_CHECK(cudaMemcpy(&max_random_absolute_error, d_max_random_absolute_error, sizeof(float), cudaMemcpyDeviceToHost));
  ...
  std::printf(
      "overlap: GEMM -> optimized checksum -> tile-local ABFT; "
  ...
  std::printf(
      "abft_verification: %s\n",
      bad_tiles == 0 ? "PASS" : (options.inject_fault && bad_tiles == 1 && corrected_tiles == 1 ? "PASS_AFTER_SINGLE_CORRECTION" : "FAIL"));
