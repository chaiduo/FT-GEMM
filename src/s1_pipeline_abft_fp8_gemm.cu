...
static Options parse_options(int argc, char** argv) {
  ...
  if (options.m <= 0 || options.n <= 0 || options.k <= 0 ||
      ...
      options.verify_samples < 0 || options.batch_tiles <= 0 ||
...
__global__ void native_tiled_fp8_gemm_kernel(
    const Element* a, const Element* b, float* c, int m, int n, int k,
    int tile_begin, int tile_count, int tile_cols) {
  ...
  for (int k_base = 0; k_base < k; k_base += kTile) {
    ...
    a_tile[threadIdx.y][threadIdx.x] =
        row < m && a_col < k ? a[row * k + a_col] : Element(0.0f);
    b_tile[threadIdx.y][threadIdx.x] =
        b_row < k && col < n ? b[b_row * n + col] : Element(0.0f);
    __syncthreads();
    for (int kk = 0; kk < kTile; ++kk) {
      accumulator += static_cast<float>(a_tile[threadIdx.y][kk]) *
                     static_cast<float>(b_tile[kk][threadIdx.x]);
    }
    __syncthreads();
  }
  if (row < m && col < n) {
...
__global__ void input_tile_sums_kernel(
    const Element* a, const Element* b, float* a_sums, float* b_sums,
    int m, int n, int k, int tile_rows, int tile_cols) {
...
__device__ __forceinline__ bool checksum_bad(
    float actual, float expected, float abs_tolerance, float rel_tolerance) {
...
__global__ void tile_checksum_kernel(
    ...
    const float* b_sums, float* c, float* row_deltas, float* col_deltas,
    int m, int n, int k, int tile_rows, int tile_cols, int tile_begin,
    int tile_count, float abs_tolerance, float rel_tolerance,
    bool inject_fault) {
  ...
  if (inject_fault && threadIdx.x == 0 &&
      tile_id == (tile_rows * tile_cols) / 2) {
  ...
  for (int index = threadIdx.x; index < kTile * kTile; index += kThreads) {
    ...
    c_tile[index] =
        global_row < m && global_col < n ? c[global_row * n + global_col]
                                         : 0.0f;
  }
  __syncthreads();
  ...
  if (threadIdx.x < valid_rows) {
    ...
    for (int kk = 0; kk < k; ++kk) {
      expected += static_cast<float>(a[global_row * k + kk]) *
                  b_sums[tile_col * k + kk];
    }
  ...
  if (threadIdx.x < valid_cols) {
    ...
    for (int kk = 0; kk < k; ++kk) {
      expected += a_sums[tile_row * k + kk] *
                  static_cast<float>(b[kk * n + global_col]);
    }
    col_deltas[tile_id * kTile + threadIdx.x] = expected - actual;
  }
}

__global__ void tile_abft_kernel(
    float* c, const float* row_deltas, const float* col_deltas, int m, int n,
    int tile_rows, int tile_cols, int tile_begin, int tile_count,
    float abs_tolerance, float rel_tolerance, int* bad_tiles,
    int* corrected_tiles) {
  ...
  if (threadIdx.x == 0 && bad_row_count == 1 && bad_col_count == 1) {
    ...
    c[row * n + col] +=
        row_deltas[tile_id * kTile + bad_row] * 0.5f +
        col_deltas[tile_id * kTile + bad_col] * 0.5f;
    atomicAdd(corrected_tiles, 1);
  }
}

static float verify_samples(const std::vector<Element>& a,
    ...
    for (int col = 0; col < options.n; col += col_step) {
      ...
      for (int kk = 0; kk < options.k; ++kk) {
        reference += static_cast<double>(
                     ...
                     static_cast<double>(
                         static_cast<float>(b[kk * options.n + col]));
      }
      maximum = std::max(
          maximum,
          static_cast<float>(
              std::fabs(c[row * options.n + col] - reference)));
    }
  }
  return maximum;
}
...
int main(int argc, char** argv) {
  ...
  CUDA_CHECK(cudaMalloc(&d_row_deltas,
                        tile_count * kTile * sizeof(float)));
  CUDA_CHECK(cudaMalloc(&d_col_deltas,
                        tile_count * kTile * sizeof(float)));
  ...
  input_tile_sums_kernel<<<std::max(tile_rows, tile_cols), threads, 0,
                           checksum_stream>>>(
      d_a, d_b, d_a_sums, d_b_sums, options.m, options.n, options.k,
      tile_rows, tile_cols);
  CUDA_CHECK(cudaEventRecord(metadata_done, checksum_stream));
  CUDA_CHECK(cudaEventSynchronize(metadata_done));

  int batch_count = (tile_count + options.batch_tiles - 1) /
                    options.batch_tiles;
  ...
  auto enqueue_iteration = [&]() {
    for (int batch = 0; batch < batch_count; ++batch) {
      ...
      native_tiled_fp8_gemm_kernel<<<tiles_this_batch, gemm_block, 0,
                                     gemm_stream>>>(
          d_a, d_b, d_c, options.m, options.n, options.k, tile_begin,
          tiles_this_batch, tile_cols);
      ...
      tile_checksum_kernel<<<tiles_this_batch, kThreads, kTile * kTile *
                             sizeof(float), checksum_stream>>>(
          d_a, d_b, d_a_sums, d_b_sums, d_c, d_row_deltas, d_col_deltas,
          options.m, options.n, options.k, tile_rows, tile_cols, tile_begin,
      ...
      tile_abft_kernel<<<tiles_this_batch, kThreads, 0, abft_stream>>>(
          d_c, d_row_deltas, d_col_deltas, options.m, options.n, tile_rows,
  ...
  for (int i = 0; i < options.repeat; ++i) {
    ...
    for (int batch = 0; batch < batch_count; ++batch) {
      float value = 0.0f;
      CUDA_CHECK(cudaEventElapsedTime(&value, gemm_start[batch],
                                      gemm_done[batch]));
      gemm_ms += value;
      CUDA_CHECK(cudaEventElapsedTime(&value, checksum_start[batch],
                                      checksum_done[batch]));
      checksum_ms += value;
      CUDA_CHECK(cudaEventElapsedTime(&value, abft_start[batch],
                                      abft_done[batch]));
      abft_ms += value;
    }
  }
  ...
  double tflops =
      ...
      (average_total_ms * 1.0e9);
  float error = verify_samples(h_a, h_b, h_c, options);
  bool verification = options.inject_fault
                          ? bad_tiles == 1 && corrected_tiles == 1
                          : bad_tiles == 0 && error < 1.0e-3f;
